...
#SBATCH --time=15:00:00               # Wall-clock time limit (HH:MM:SS)
...
conda activate mdm
python -m train.train_mdm --save_dir save_latest/my_humanml_trans_dec_bert_512_hdepic_v4_CA_lossawaresampler_weight_heavy --dataset HDEPIC --diffusion_steps 50 --arch trans_dec --text_encoder_type bert --mask_frames --use_ema --wandb_name HDEPIC_train_latest_lossawaresampler_weight_heavy --lr 1e-6 --overwrite
"
# python -m train.train_mdm --save_dir save_latest/my_humanml_trans_dec_bert_512_hdepic_v4_CA_lossawaresampler_humanML3D_pretrained --dataset HDEPIC --diffusion_steps 50 --arch trans_dec --text_encoder_type bert --mask_frames --use_ema --wandb_name HDEPIC_train_latest_lossawaresampler_humanML3D_pretrained --lr 1e-6 --overwrite
# python -m train.train_mdm --save_dir save_latest/my_humanml_trans_dec_bert_512_hdepic_v4_CA_lossawaresampler_temporal_weighted_heavy --dataset HDEPIC --diffusion_steps 50 --arch trans_dec --text_encoder_type bert --mask_frames --use_ema --wandb_name HDEPIC_train_latest_lossawaresampler_temporal_weighted_heavy --lr 1e-6 --overwrite 
